...
async function main() {
  ...
  const MMCreativeNFTContractFactory = await ethers.getContractFactory(
    "MMCreativeNFTContract"
  );
  // deploy
  const MMCreativeNFTContract = await MMCreativeNFTContractFactory.deploy(
    args.mint_price,
    args.max_tokens,
    args.base_uri,
    args.royaltyArtist,
  ...
  await MMCreativeNFTContract.deploymentTransaction().wait(15);
  ...
  for (let i = 0; i < 3; i++) {
    const transactionResponse = await MMCreativeNFTContract.mintTo(ipfs[i], {
