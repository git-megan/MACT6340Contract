...
describe("MMCreativeNFTContract", async function () {
  let MMCreativeNFTContractFactory;
  let MMCreativeNFTContract;
  let args = {
  ...
  this.beforeEach(async function () {
    MMCreativeNFTContractFactory = await ethers.getContractFactory(
      "MMCreativeNFTContract"
    );
    MMCreativeNFTContract = await MMCreativeNFTContractFactory.deploy(
      args.mint_price,
      args.max_tokens,
      args.base_uri,
      args.royaltyArtist,
    ...
    await MMCreativeNFTContract.waitForDeployment(
      args.mint_price,
      args.max_tokens,
      args.base_uri,
      args.royaltyArtist,
  ...
  describe("construction and initialization", async function () {
    ...
    it("should be named MMCreativeNFTContract", async function () {
      const expectedValue = "MMCreativeNFTContract";
      const currentValue = await MMCreativeNFTContract.name();
      console.log(currentValue.toString(), expectedValue);
      assert.equal(currentValue.toString(), expectedValue);
    });
    it("should be have symbol MMC", async function () {
      const expectedValue = "MMC";
      const currentValue = await MMCreativeNFTContract.symbol();
      assert.equal(currentValue.toString(), expectedValue);
    });
    it("should have a mint price set when constructed", async function () {
      const expectedValue = args.mint_price;
      const currentValue = await MMCreativeNFTContract.getMintPrice();
      assert.equal(currentValue.toString(), expectedValue);
    });
    it("should have a max token supply set when constructed", async function () {
      const expectedValue = args.max_tokens;
      const currentValue = await MMCreativeNFTContract.getMaxSupply();
      assert.equal(currentValue.toString(), expectedValue);
    });

    it("should have a base URI set when constructed", async function () {
      const expectedValue = args.base_uri;
      const currentValue = await MMCreativeNFTContract.getBaseURI();
      assert.equal(currentValue.toString(), expectedValue);
    });
    ...
    it("should set roylaty artist when constructed", async function () {
      ...
      const currentValue = await MMCreativeNFTContract.royaltyInfo(
        1,
    ...
    it("should set owner to the deployer's address when constucted", async function () {
      ...
      const currentValue = await MMCreativeNFTContract.owner();
      assert.equal(currentValue.toString(), expectedValue);
    });
  });
  ...
  describe("receive function", async function () {
    ...
    it("should be called and revert if called from low-level transaction", async function () {
      let contractAddress = await MMCreativeNFTContract.getAddress();
  ...
  describe("mintTo function", async function () {
    ...
    it("should revert if called with no ether", async function () {
      const [owner, artist, buyer] = await ethers.getSigners();
      expect(
        MMCreativeNFTContract.connect(buyer).mintTo(args.base_uri, {
    ...
    it("should have a totalSupply = 1 with after first mint", async function () {
      ...
      const mint1 = await MMCreativeNFTContract.connect(buyer).mintTo(
        args.base_uri,
        { value: args.mint_price }
      );
      mint1.wait(1);
      const currentValue = await MMCreativeNFTContract.totalSupply();
      assert.equal(currentValue.toString(), expectedValue);
    });
    ...
    it("should increase the totalSupply by 1 with with each mint", async function () {
      ...
      mint1.wait(1);
      const mint2 = await MMCreativeNFTContract.connect(buyer).mintTo(
        args.base_uri,
        { value: args.mint_price }
      );
      mint2.wait(1);
    ...
    it("should emit an event when minting is completed", async function () {
      ...
      expect(
        ...
        .to.emit(MMCreativeNFTContract, "MintingCompleted")
        .withArgs(1, owner);
    });
    ...
    it("should have set the token uri during minting function", async function () {
      ...
      mint1.wait(1);
      const currentValue = await MMCreativeNFTContract.tokenURI(1);
      assert.equal(currentValue.toString(), expectedValue);
    });
    ...
    it("should emit an event after funds are distributed during mint", async function () {
      ...
      const uriString = "someString";
      expect(
        MMCreativeNFTContract.connect(buyer).mintTo(uriString, {
          value: args.mint_price,
        })
      )
        .to.emit(MMCreativeNFTContract, "FundsDistributed")
